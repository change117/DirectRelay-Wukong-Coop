...
/// required by the in-game WukongMp.Coop mod.
///
...
class MockApiServer
{
    ...
    public MockApiServer(int port, Action<string, string, ConsoleColor> log)
    {
        ...
        // Start with localhost — works without admin on most Windows configs
    ...
    async Task HandleRequest(HttpListenerContext context)
    {
        ...
        try
        {
            ...
            // Returns download URL for the requested save file
            ...
            // Returns a URL where the game can PUT the save file
            ...
            // Catch-all: return 200 OK with empty response
            // This prevents the game from crashing on unexpected requests
    ...
    /// <summary>
    /// GET /api/server/{id}/files/{filename}
    /// Game expects: { "downloadUrl": "http://..." } or { "downloadUrl": null }
    /// </summary>
    async Task HandleDownloadMetadata(string path, HttpListenerResponse resp)
    {
        // Extract filename from path: /api/server/1/files/world.sav
        ...
        // If we don't have it yet, try loading from seed save
        ...
        {
            // No save exists — game will start a new game (this is fine for first run)
    ...
    /// <summary>
    /// GET /api/server/{id}/files/upload-sas?kind=WorldSave&userGuid=xxx&serverId=1
    /// Game expects: a JSON string (quoted URL) pointing to where it can PUT the save data
    /// </summary>
    async Task HandleUploadSas(string path, string query, HttpListenerResponse resp)
    {
        // Parse query params to determine filename (manual parsing to avoid System.Web dependency)
        ...
        // Response is a bare JSON string (e.g., "http://localhost:8080/blobs/world.sav")
    ...
    /// <summary>
    /// PUT /blobs/{filename}
    /// Accepts (potentially gzip'd) save data from the game and stores it
    /// </summary>
    async Task HandleBlobUpload(string path, HttpListenerRequest req, HttpListenerResponse resp)
    {
        ...
        // The game sends gzip-compressed data; decompress for storage
        byte[] saveData;
        try
        {
